...
export default function page() {
    ...
    useEffect(() => {
        axios.post(`${apiBaseUrl}admin/team/edit/${id}`)
            .then((res) => res.data)
            .then((finalRes) => {
                setFormData({
                    memberName: finalRes.data.memberName,
                    memberCategory: finalRes.data.memberCategory,
                    status: finalRes.data.status ? 1 : 0
                })


                setPreviewImage(`${apiBaseUrl}` + finalRes.staticPath + finalRes.data.memberImage)
            })
    ...
    // updated wroked here
    let teamUpdate = (event) => {
        event.preventDefault()
        setLoader(true)
        let formData = new FormData(event.target)
        axios.put(`${apiBaseUrl}admin/team/update/${id} `, formData)
            .then((res) => {
                if (res.data.status) {
    ...
    return (
        <>
            ...
            <section className='' >
                ...
                {/* form-body */}
                <form onSubmit={teamUpdate} className=' flex flex-col gap-6 lg:px-6 px-4 py-4'  >
